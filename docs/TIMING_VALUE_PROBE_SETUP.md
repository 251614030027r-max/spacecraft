...
3. **翻滚速率变化(执行单 §2 第 4 项)本轮不实现,建议推迟。** 它是四个放开旋钮里
   **唯一触碰控制难度**的(共旋力 ∝ ω²r);而回答"择时值不值钱"**不需要**它——相位分散
   ...
   请两窗口确认是否接受。**
4. **B 臂是真值-轨迹 oracle,不是反应式阈值、也不是恒体率外推。** 见 §3。理由:一次接近
   ...
   不可信,会让"预测器本身"而非"择时"限制结果。这是 oracle = 择时价值上界,**不是**实时控制器
   (正式方法不享受 oracle);本探针就是测"知道未来有利相位后择时值不值钱"。
...
## 3. B 臂 oracle 逻辑(`ScriptedEntryTiming`)
...
缓存轨迹与实际步进逐位一致(目标自由翻滚、不受控),所以 A/B 见到的目标动力学完全相同,只有
oracle 的前瞻不同。
