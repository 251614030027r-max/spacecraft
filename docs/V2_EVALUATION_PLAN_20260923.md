...
**总量约 396 个 episode。按下面的并行规则跑，挡路的部分约 2.5–3 小时。**
...
| 任何字段不一致 | **并行结果作废**，第 1 步串行重跑（4.8 小时），并把差值记进报告 |

挡路路径的耗时：第 00 步约 20 分钟（改代码 + 两个测试）→ 第 0.1 步几分钟 →
第 0.2 步 8 分钟（抽检通过则跳过 48 回合重跑）→ 第 1 步三进程并行约 1.6 小时 →
第 2 步并行约 10 分钟 → 并行自检 8 分钟 → 第 2b 步 2 分钟。**合计约 2.5 小时。**
第 3 步条件执行，四进程并行约 1.6 小时，在上报之后跑。
...
**这里省掉 1.6 小时，靠的是抽检而不是省判据。** 先跑 4 个 episode，
与旧的 `pure_mpc_nominal.json` 的同种子回合逐字段比：
...
重跑命令（仅在上表第二行触发时跑，48 episode，与第 1 步并行，约 1.6 小时）：
...
## 第 1 步：主表，nominal，48 种子（144 episode，三进程并行约 1.6 小时）
...
for SEED in 262410 262411 262412; do
  ...
  nohup python -u -B -m experiments.evaluate_hybrid_policy --episodes 48 --seed 262000 $FLAGS \
    --model logs/v2_${SEED}/final_model.zip \
    --output eval/adp/final_v2_nominal_${SEED}.json \
    > eval/adp/log_v2_nominal_${SEED}.txt 2>&1 &
done
wait
```
...
## 第 2 步：架构地板复验（12 episode，三进程并行约 10 分钟）
...
## 第 3 步：跨工况（条件执行，**放在上报之后**，192 episode，四进程并行约 1.6 小时）
